...
// Ported to Linux by David (B1QUAD). Last modified 04/21/2022
...
int main(void) {
	...
	if ((bmpDataOff - bmpDIBSize) != 14) {
		fprintf(stderr, "Warning: This file's data offset is %ld bytes (expected %ld bytes).\n", bmpDataOff, (bmpDIBSize + 14));
	}
	if (bmpDIBSize > 40) {
		fprintf(stderr, "Warning: This file's header is %ld bytes (expected 40 bytes), which may result in other errors.\n", bmpDIBSize);
	}
	if (bmpDIBSize < 40) {
		fprintf(stderr, "Error: This file's header is %ld bytes (expected 40 bytes).\n", bmpDIBSize);
		fclose(in);
		fclose(out);
		scanf("%s", &data);
		return 0;
	...
	if (bmpWidth > 128) {
		fprintf(stderr, "Warning: This image's width of %ld exceeds the ST7735's 128 pixel width.\n", bmpWidth);
	}
	...
	if (bmpHeight > 160) {
		fprintf(stderr, "Warning: This image's height of %ld exceeds the ST7735's 160 pixel height.\n", bmpHeight);
	}
	if (bmpHeight < 0) {
